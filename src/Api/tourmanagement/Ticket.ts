...
import type { TicketResponse } from "@/interface/tour/TicketResponse";
...
export const ticketService = {
  getAll: async (): Promise<Ticket[]> => {
    const response =
      await apiClient.get<ApiResponse<TicketResponse[]>>("/tickets");
  ...
  updateStatus: async (id: number, status: GenericStatus): Promise<Ticket> => {
    const response = await apiClient.put<ApiResponse<TicketResponse>>(
      `/tickets/${id}/status`,
      null,
      {
        params: { status },
